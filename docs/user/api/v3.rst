...
.. note::
   ...
   Authenticating your API requests with a :ref:`token <api/v3:Token>` also bypasses most other rate limiting,
   including network-level blocks that may affect automated scripts accessing Read the Docs from cloud providers or CI environments.
   If your script is getting ``403 Forbidden`` responses, adding an ``Authorization: Token <your-access-token>`` header is the recommended solution.
...
These endpoints let you host documentation that was built outside Read the Docs.
They are used by the `GitHub upload action <https://github.com/readthedocs/upload-action>`__
and the `command line client <https://github.com/readthedocs/readthedocs-cli>`__,
which is the recommended way to use them.

Uploading is done in three steps:

#. Initiate the upload to create the build and get a pre-signed URL for the storage.
#. Upload a zip file with the documentation to that URL.
#. Complete the upload to start processing the build.

The zip file contains the HTML at ``html/``,
and optionally a single file under ``pdf/``, ``epub/`` and ``htmlzip/``.

.. note::

   Uploading is enabled project by project.
   :doc:`Contact support </support>` to enable it on your project.
   See :doc:`/guides/build/upload-documentation` for the full guide.

Upload initiate
+++++++++++++++

.. http:post:: /api/v3/upload/initiate/

    Create a build in the ``triggered`` state for the given version,
    creating the version if it does not exist,
    and return a pre-signed URL to upload the documentation.
    ...
    **Example request**:
    ...
    .. tabs::
        ...
        .. code-tab:: bash
            ...
            $ curl \
              -X POST \
              -H "Authorization: Token <token>" https://app.readthedocs.org/api/v3/upload/initiate/ \
              -H "Content-Type: application/json" \
              -d @body.json
        ...
        .. code-tab:: python
            ...
            import requests
            URL = 'https://app.readthedocs.org/api/v3/upload/initiate/'
            TOKEN = '<token>'
            HEADERS = {'Authorization': f'token {TOKEN}'}
            data = {
                'project': 'pip',
    ...
    **Example response**:
    ...
    :>json string project: slug of the project.
    :>json object version: ``name`` (branch name, tag name or pull request number),
        ``type`` (``branch``, ``tag`` or ``external``),
        ``commit`` (full commit hash) and
        ``privacy_level`` (``public`` or ``private``, |com_brand| only).
    :statuscode 201: the build was created
    :statuscode 403: the user is not an admin of the project, uploading is not enabled, or the project is not active
    :statuscode 404: the project was not found
    :statuscode 429: the project has too many pending uploads

    Upload the zip file to ``upload_url.url`` with a ``multipart/form-data`` POST request,
    sending every entry of ``upload_url.fields`` as a form field,
    plus the zip file in the ``file`` field.
    The URL is valid for 30 minutes and accepts files up to 1 GB.
    Do not send the ``Authorization`` header to the storage.
    ...
    .. tabs::
        ...
        .. code-tab:: bash
            ...
            $ curl \
              ...
              -F "policy=..." \
              -F "x-amz-signature=..." \
              -F "file=@artifacts.zip;type=application/zip" \
              https://<storage>/
        ...
        .. code-tab:: python
            ...
            with open('artifacts.zip', 'rb') as fh:
                response = requests.post(
                    upload_url['url'],
                    data=upload_url['fields'],
                    files={'file': ('artifacts.zip', fh, 'application/zip')},
                )
            response.raise_for_status()

    The form fields have to be sent exactly as returned by the initiate endpoint,
    and before the ``file`` field.

Upload complete
+++++++++++++++

.. http:post:: /api/v3/upload/complete/

    Notify Read the Docs that the upload finished.
    On success, the build starts processing the uploaded files.
    On failure, the build is marked as failed.
    ...
    **Example request**:
    ...
    .. tabs::
        ...
        .. code-tab:: bash
            ...
            $ curl \
              -X POST \
              -H "Authorization: Token <token>" https://app.readthedocs.org/api/v3/upload/complete/ \
              -H "Content-Type: application/json" \
              -d '{"build": 12345, "status": "success"}'

        .. code-tab:: python

            import requests
            URL = 'https://app.readthedocs.org/api/v3/upload/complete/'
            TOKEN = '<token>'
            HEADERS = {'Authorization': f'token {TOKEN}'}
            data = {'build': 12345, 'status': 'success'}
            response = requests.post(URL, json=data, headers=HEADERS)
            print(response.json())
    ...
    **Example response**:
    ...
    :>json integer build: ID of the build returned by the initiate endpoint.
    :>json string status: ``success`` or ``failed``.
    :statuscode 202: the build was queued for processing
    :statuscode 200: the upload was reported as failed and the build was marked as failed
    :statuscode 400: the zip file was not found in the storage
    :statuscode 403: the user is not an admin of the project
    :statuscode 404: the build was not found, or it was not created by the initiate endpoint
    :statuscode 409: the build is already being processed
...
.. http:post:: /api/v3/projects/(str:project_slug)/redirects/
    ...
    **Example request**:
    ...
    .. note::
       ...
       - ``type`` (required) can be one of ``page``, ``exact``, ``clean_url_to_html`` and ``html_to_clean_url``.
       ...
       - ``position`` (optional, default: ``0``) - Order of execution. Position starts at 0 and is used to order redirects.
       - ``force`` (optional, default: ``false``) - Apply the redirect even if the page exists.
       - ``enabled`` (optional, default: ``true``) - Enable or disable the redirect.
       - ``description`` (optional) - A description for the redirect.
       - ``http_status`` (optional, default: ``302``) - HTTP status code, can be ``301`` (permanent) or ``302`` (temporary).
...
Sharing methods allow you to grant temporary access to private documentation by using a secret link, a password, or an HTTP header token.
See :doc:`/commercial/sharing` for more information.
...
.. http:get:: /api/v3/projects/(str:project_slug)/sharing/(int:sharing_id)/

    Retrieve details of a single sharing method for a project.
    ...
    **Example request**:
    ...
    .. tabs::
        ...
        .. code-tab:: bash
            ...
            $ curl -H "Authorization: Token <token>" https://app.readthedocs.com/api/v3/projects/pypa-pip/sharing/1/

        .. code-tab:: python

            import requests
            URL = 'https://app.readthedocs.com/api/v3/projects/pypa-pip/sharing/1/'
            TOKEN = '<token>'
            HEADERS = {'Authorization': f'token {TOKEN}'}
            response = requests.get(URL, headers=HEADERS)
            print(response.json())
    ...
    **Example response**:
    ...
    .. sourcecode:: json
       ...
       {
           "_links": {
               "_self": "https://app.readthedocs.com/api/v3/projects/pypa-pip/sharing/1/",
               "project": "https://app.readthedocs.com/api/v3/projects/pypa-pip/"
           },
           "access_type": "token",
           "allow_all": true,
           ...
           "url": "https://pypa-pip.readthedocs-hosted.com/_/sharing/<generated-token>?next=/en/latest/",
           "versions": []
       }


Sharing methods listing
+++++++++++++++++++++++

.. http:get:: /api/v3/projects/(str:project_slug)/sharing/

    Retrieve list of all sharing methods for this project.
    ...
    **Example request**:
    ...
    .. tabs::
        ...
        .. code-tab:: bash
            ...
            $ curl -H "Authorization: Token <token>" https://app.readthedocs.com/api/v3/projects/pypa-pip/sharing/

        .. code-tab:: python

            import requests
            URL = 'https://app.readthedocs.com/api/v3/projects/pypa-pip/sharing/'
            TOKEN = '<token>'
            HEADERS = {'Authorization': f'token {TOKEN}'}
            response = requests.get(URL, headers=HEADERS)
            print(response.json())
    ...
    **Example response**:
    ...
    .. sourcecode:: json

        {
            ...
            "next": null,
            "previous": null,
            "results": ["SHARING METHOD"]
        }


Sharing method create
+++++++++++++++++++++

.. http:post:: /api/v3/projects/(str:project_slug)/sharing/

    Create a sharing method for this project.
    ...
    **Example request**:
    ...
    .. tabs::
        ...
        .. code-tab:: bash
            ...
            $ curl \
              -X POST \
              -H "Authorization: Token <token>" https://app.readthedocs.com/api/v3/projects/pypa-pip/sharing/ \
              -H "Content-Type: application/json" \
              -d @body.json
    ...
    .. note::
       ...
       - ``access_type`` can be ``token``, ``password``, or ``http_header_token``.
       - ``password`` is required only when ``access_type`` is ``password``.
       - ``allow_all`` defaults to ``true``.
       - When ``allow_all`` is ``false``, ``versions`` must include at least one private active version slug.
       - ``expires`` defaults to 7 days from creation.

    **Example response**:

    `See Sharing method details <#sharing-method-details>`_.

    :statuscode 201: Sharing method created successfully.


Sharing method update
+++++++++++++++++++++

.. http:patch:: /api/v3/projects/(str:project_slug)/sharing/(int:sharing_id)/

    Update a sharing method for this project.
    ...
    **Example request**:
    ...
    .. tabs::
        ...
        .. code-tab:: bash
            ...
            $ curl \
              -X PATCH \
              -H "Authorization: Token <token>" https://app.readthedocs.com/api/v3/projects/pypa-pip/sharing/1/ \
              -H "Content-Type: application/json" \
              -d @body.json
    ...
    .. sourcecode:: json
        ...
        {
            "description": "Updated support team access"
        }
...
.. http:delete:: /api/v3/projects/(str:project_slug)/sharing/(int:sharing_id)/

    Delete a sharing method for this project.
    ...
    **Example request**:
    ...
    :statuscode 204: Sharing method deleted successfully.
