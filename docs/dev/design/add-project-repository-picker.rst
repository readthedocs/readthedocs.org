...
The main action is a search input, but it is easy to miss.
It looks like a list filter,
and the placeholder box below it, which points the user back to the search, takes up most of the page.
...
The dropdown works well for the first part and less well for the other two.
Results close on blur, so the user can't compare two repos or read the state icons at leisure,
...
Keep the search dropdown, make it the hero
   ...
   It doesn't address the transient results or the repair flow,
   so it improves the page without changing what makes it hard to use.
...
A list with a filter
   ...
   That's acceptable, since a user with 300 repos usually knows the name.
...
Group by account and organization
   ...
   and a mostly empty sidebar for someone with one account and five repos,
...
Everywhere else in the dashboard a search dropdown is used to jump somewhere, not to choose between items.
That is most of why the page feels different from the rest of the dashboard.
...
The hero-search option keeps the current component, so it doesn't close that gap.
...
and it costs about the same as the grouped layout without the extra layout for small accounts.
