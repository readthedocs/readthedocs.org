...
class TestTasks(TestCase):
    def test_delete_revoked_keys(self):
        ...
        revoked_key = get(
            ProjectAPIKey, project=project, revoked=True, name="revoked key"
        )
        expired_key = get(
            ProjectAPIKey,
            project=project,
        ...
        valid_key = get(ProjectAPIKey, project=project, name="valid key")
        ...
        self.assertEqual(ProjectAPIKey.objects.count(), 6)
        delete_old_revoked_build_api_keys(days=7)
        self.assertEqual(ProjectAPIKey.objects.count(), 4)
        deleted_keys = [
            expired_old_key,
            revoked_old_key,
        ]
        self.assertFalse(
            ProjectAPIKey.objects.filter(pk__in=[key.pk for key in deleted_keys]).exists()
        )
