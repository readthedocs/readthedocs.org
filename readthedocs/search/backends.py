...
class BackendV1(Backend):
    ...
    def __init__(self, *, request, query, project, version):
    ...
    def _get_projects_to_search(self):
        if not self._has_permission(self.request.user, self.version):
            return

        yield self.project, self.version

        yield from self._get_subprojects(self.project, version_slug=self.version.slug)
