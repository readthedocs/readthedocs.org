...
class ValidateChoice:
    ...
    def it_accepts_valid_choice(self):
    ...
    def it_rejects_invalid_choice(self):
        with raises(ValidationError) as excinfo:
...
class ValidateList:
    ...
    def it_accepts_list_types(self):
    ...
    def it_rejects_string_types(self):
        with raises(ValidationError) as excinfo:
            result = validate_list('choice')
        assert excinfo.value.code == INVALID_LIST


class ValidateDirectory:

    def it_uses_validate_path(self, tmpdir):
    ...
    def it_rejects_files(self, tmpdir):
...
class ValidateFile:
    ...
    def it_rejects_directories(self, tmpdir):
...
class ValidateString:
    ...
    def it_accepts_unicode(self):
        result = validate_string(u'Unicöde')
        assert isinstance(result, text_type)

    def it_accepts_nonunicode(self):
        result = validate_string('Unicode')
        assert isinstance(result, text_type)

    def it_rejects_float(self):
        with raises(ValidationError) as excinfo:
            validate_string(123.456)
        assert excinfo.value.code == INVALID_STRING

    def it_rejects_none(self):
        with raises(ValidationError) as excinfo:
