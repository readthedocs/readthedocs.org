    ...
    def get(self, request):  # noqa
        ...
        if external:
            ...
            if cache.get(cache_key) > settings.RTD_EMBED_API_DOMAIN_RATE_LIMIT:
                log.warning(
                    'Too many requests for this domain. domain=%s', domain
                )
                return Response(
                    {
                        'error': (
        ...
        try:
            ...
            log.exception(
                'There was an error reading the URL requested. url=%s', url
            )
            return Response(
                {
                    'error': (
        ...
        if not content_requested:
            log.warning(
                'Identifier not found. url=%s fragment=%s', url, fragment
            )
            return Response({
                'error': (
