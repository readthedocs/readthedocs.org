...
class ReadTheDocsRequestMiddleware(RequestMiddleware):
    ...
    """
    ``ReadTheDocsRequestMiddleware`` adds request metadata to ``structlog``'s
    logger context.
    ...
    def __call__(self, logger, name, event_dict):
        ...
        if level is not None:
            sio.write(
                ...
                _pad(level, self._longest_level) + self._styles.reset + '] '
            )
        ...
        if logger_name is not None:
            sio.write(
                ...
                logger_name + self._styles.reset + ':' + str(line_number) + '] '
            )
        ...
        sio.write(
            ' '.join(
                ...
                self._styles.reset for key in event_dict_keys
            )
        )
