...
class TestBuildReadthedocsYamlData:
    ...
    def test_build_config_falls_back_to_readthedocs_yaml_config(self):
        """Test Build.config falls back to readthedocs_yaml_config when _config is missing."""
        project = fixture.get(Project)
        config_data = {"build": {"os": "ubuntu-22.04"}, "python": {"version": "3.11"}}
        ...
        Build.objects.filter(pk=build.pk).update(_config=None)
        build.refresh_from_db()

        assert build._config is None
        assert build.readthedocs_yaml_config is not None
        assert build.config == config_data

    def test_build_config_reference_falls_back_to_referenced_readthedocs_yaml_config(self):
        """
        Test Build.config follows __config references and falls back to BuildConfig data.
        """
        project = fixture.get(Project)
        version = fixture.get(Version, project=project)
        config_data = {"build": {"os": "ubuntu-22.04"}, "python": {"version": "3.11"}}
        ...
        Build.objects.filter(pk=build1.pk).update(_config=None)
        build2.refresh_from_db()

        assert build2.config == config_data
