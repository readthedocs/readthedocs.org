...
class RedirectTests(BaseDocServing):
    ...
    def test_subproject_root_url(self):
        r = self.client.get('/projects/subproject/', secure=True, HTTP_HOST='project.dev.readthedocs.io')
        self.assertEqual(r.status_code, 302)
        self.assertEqual(
    ...
    def test_subproject_redirect(self):
        ...
        r = self.client.get('/en/latest/', secure=True, HTTP_HOST='subproject.dev.readthedocs.io')
        self.assertEqual(r.status_code, 302)
        self.assertEqual(
            r['Location'], 'https://project.dev.readthedocs.io/projects/subproject/en/latest/',
        )

        r = self.client.get('/en/latest/foo/bar', secure=True, HTTP_HOST='subproject.dev.readthedocs.io')
        self.assertEqual(r.status_code, 302)
        self.assertEqual(
    ...
    def test_single_version_subproject_redirect(self):
        ...
        r = self.client.get('/foo/bar/', secure=True, HTTP_HOST='subproject.dev.readthedocs.io')
        self.assertEqual(r.status_code, 302)
        self.assertEqual(
    ...
    def test_root_redirect_with_query_params(self):
        r = self.client.get('/?foo=bar', secure=True, HTTP_HOST='project.dev.readthedocs.io')
        self.assertEqual(r.status_code, 302)
        self.assertEqual(
            r['Location'],
    ...
    def test_canonicalize_public_domain_to_cname_redirect(self):
        ...
        # We should redirect before 404ing
        r = self.client.get('/en/latest/404after302', secure=True, HTTP_HOST='project.dev.readthedocs.io')
        self.assertEqual(r.status_code, 302)
        self.assertEqual(
            r['Location'], f'https://{self.domain.domain}/en/latest/404after302',
        )
    ...
    def test_translation_redirect(self):
        r = self.client.get('/', secure=True, HTTP_HOST='translation.dev.readthedocs.io')
        self.assertEqual(r.status_code, 302)
        self.assertEqual(
    ...
    def test_https_public_domain_https_redirect(self):
        ...
        for path in paths:
            r = self.client.get(path, secure=False, HTTP_HOST='project.dev.readthedocs.io')
            self.assertEqual(r.status_code, 302)
            self.assertEqual(
                r['Location'], f'https://project.dev.readthedocs.io{path}',
            )
            self.assertEqual(r["X-RTD-Redirect"], RedirectType.http_to_https.name)
