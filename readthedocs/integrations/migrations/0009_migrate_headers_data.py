...
def forwards_func(apps, schema_editor):
    ...
    (
        HttpExchange.objects.annotate(
            request_headers_in_json=Cast(
                "request_headers", output_field=models.JSONField()
            ),
            response_headers_in_json=Cast(
                "response_headers", output_field=models.JSONField()
            ),
        ).update(
    ...
    (
        Integration.objects.annotate(
            provider_data_in_json=Cast(
                "provider_data", output_field=models.JSONField()
            ),
        ).update(
